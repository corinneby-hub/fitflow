...
const Api = (() => {
  ...
  function summarizeRecentSuggestions(recent) {
    const names = [...new Set((recent || []).flatMap(r => r.names))];
    return names;
  }

  return {
    async generateWorkout({ settings, history, recent, comment, minutes, focus }) {
      const recentNames = summarizeRecentSuggestions(recent);
      const userMessage = [
        ...
        recentNames.length
          ? [
              ...
              recentNames.join(", "),
              "HARD VARIETY RULE: at least half of today's MAIN exercises must NOT appear on that list (different exercises, not just renamed variations). Pick fresh alternatives that still fit the equipment, level and focus. Warmup/cooldown may repeat.",
            ].join("\n")

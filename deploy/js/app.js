...
(() => {
  ...
  /* ---------------- Force refresh ---------------- */
  // Clears only the file cache — localStorage (history, settings) is untouched
  ...
  function init() {
    ...
    $("#app-version").textContent = `FitFlow ${APP_VERSION}`;

    if ("serviceWorker" in navigator) {
      const hadController = !!navigator.serviceWorker.controller;
      let reloading = false;

      // A newly activated service worker means new code — reload once to run it
